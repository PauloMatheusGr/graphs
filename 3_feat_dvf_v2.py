#!/usr/bin/env python3
"""
DVF v2: extrai atributos de warps sujeito → template CN (fixed=template).

Pré-requisito: 3_feat_gen_dvf_v2.py → images/displacement_field_v2/

Pastas / saídas:
  warps:  images/displacement_field_v2/
  out:    csvs/cohorts/all_population/features_displacement_v2.csv
  run:    images/displacement_field_v2/features/{COHORT}/

Fluxo:
  1) Lê IMAGES_CSV.
  2) Inferir ref_tag baseline por paciente (warps em disco).
  3) Para cada ID_IMG:
       - domínio = template CN (fixed do registo v2)
       - displacement via inv_list = [affine, inverseWarp] nos pontos do template
         (template → sujeito; mismatch sujeito–CN no espaço do template)
       - warpa regions + brain_mask do sujeito → template (fwd: Warp + Affine, NN)
       - agrega por ROI no espaço do template (mesmas stats que v1)
  4) Resume com done_keys.txt / OUT_CSV.
"""
...
def template_path_for(sex: str, age_range: str) -> str:
    return os.path.join(
        GROUPWISE_DIR,
        f"groupwise_DIAG-CN_SEX-{sex}_AGE-{age_range}_N-20_template.nii.gz",
    )
...
def _ref_tag(sex: str, age_range: str) -> str:
    return f"CN_SEX-{sex}_AGE-{age_range}"
...
def compute_unitary_scalar_arrays(domain_img: ants.ANTsImage, inv_list: list[str]):
    """
    Mapas escalares no espaço do template (domain_img = template CN).
    Displacement via inv_list (template → sujeito).
    """
    ...
    return (
        ...
        strain_maps,
        logjac,  # ref_img (mesma geometria do template)
    )
...
def main():
    ...
    for r in df.itertuples(index=False):
        ...
        if LOG_EVERY > 0 and (processed % LOG_EVERY) == 0:
            ...
            print(
                f"[OK] IMG={img_id} PT={id_pt} ref=CN_SEX-{sex}_AGE-{age_range} "
