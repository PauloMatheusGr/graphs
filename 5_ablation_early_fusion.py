...
# python 5_ablation_early_fusion.py --cohort 48m_12m \
#   --fusion shape:t1_only,vol:deltas_only \
#   --tasks smci_pmci --selection l1_stable --models svm --combat false --repeats 10 \
#   --tuner optuna --optuna-trials 30
#
# Early fusion = concat de features. Late fusion = 5_ablation_late_fusion.py
# Protocol na planilha = fingerprint (ex. t1_shape__deltas_vol), não "fusion".

"""Nested CV: early fusion cross-mod (concat). Saída: ablation_results_fusion/{fingerprint}/."""
...
COHORT = "48m_12m"
log = logging.getLogger("ablation_fusion")
...
def setup_logging(*, log_file: Path | None, verbose: bool) -> None:
    ...
    if log_file is not None:
        ...
        fh.setFormatter(fmt)
        root.addHandler(fh)
    logging.getLogger("ablation_fusion").setLevel(level)
    logging.getLogger("ablation_runner").setLevel(level)


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Ablação nested CV — fusão cross-mod (mod:rep,...).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--fusion", default=None, help=f"default: {DEFAULT_FUSION_SPEC}")
    ...
    p.add_argument("--partner-rep", default="deltas_only")
...
def main(argv: list[str] | None = None) -> int:
    ...
    results_dir = default_crossmod_fusion_results_dir(
        base_dir, slots, results_dir=args.results_dir,
    )
    log_path = None
    if not args.no_log_file:
        log_path = args.log_file or Path(
            f"logs/ablation_fusion_{datetime.now():%Y%m%d_%H%M%S}.log"
        )
    setup_logging(log_file=log_path, verbose=args.verbose)

    fp = fusion_fingerprint(slots)
    log.info("=== ablação fusion cross-mod ===")
    log.info("cohort: %s | fusion: %s | protocol/fingerprint: %s", args.cohort, fusion_label(slots), fp)
    log.info("out: %s", results_dir)

    t0 = time.monotonic()
    try:
        df = run_fusion_ablation_suite(
            fusion_slots=slots,
            base_dir=base_dir,
            roi=args.roi,
            tasks=args.tasks,
        ...
        log.exception("ablacao fusion falhou apos %s", fmt_duration(time.monotonic() - t0))
        return 1
    ...
    log.info("csv: %s | %s", results_dir / "ablation_results_all.csv", results_dir / "ablation_summary.csv")
