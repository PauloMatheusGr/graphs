#!/usr/bin/env python3
"""
Extrai atributos DVF de warps visita → template CN (sexo/idade do baseline).

Pré-requisito: 3_feat_gen_dvf.py → images/displacement_field/

Pastas / saídas:
  warps:  images/displacement_field/
  out:    csvs/cohorts/all_population/features_displacement.csv
  run:    images/displacement_field/features/{COHORT}/

Downstream: 4_run_post_extract.py (DISP_FEATURES = features_displacement.csv).
...
Fluxo:
  ...
  3) Para cada ID_IMG:
       ...
       - agrega por ROI:
           * percentis (mean, std, p05, p50, p95) de logjac, mag, div, ux, uy, uz,
             curlmag e tensores de strain Green–Lagrange
           * momentos (variance, skewness, kurtosis) de logjac, mag e strain_fro
             infinitesimal — mean/n vêm do bloco de percentis quando compartilham o mesmo mapa
