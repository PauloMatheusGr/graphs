#!/usr/bin/env python3
"""
DVF AD: registra cada imagem clínica no template AD estratificado (sexo/idade baseline).

Paralelo a 3.1_feat_gen_dvf_v2.py (âncora CN). Saídas isoladas:
  warps:  images/displacement_field_v3_ad/
  feats:  features_displacement_v4_ad.csv  (via 3.2_feat_dvf_v4_ad.py)

fixed = template AD; moving = clínica. DIAG/GROUP ignorados na escolha do template.
"""
...
DEFAULT_TMPDIR = "./images/displacement_field_v3_ad/_tmp_ants"
...
def run_individual_registrations(
    ...
    for idx, row in df_imgs.iterrows():
        ...
        if not os.path.isfile(fixed_path):
            print(
                f"[{idx + 1}/{n_total}] [SKIP] {img_id}: template AD ausente: {fixed_path}"
            )
            n_skip += 1
            continue
    ...
    print(
        f"[DONE] registros AD: ok={n_ok} skip={n_skip} err={n_err} "
