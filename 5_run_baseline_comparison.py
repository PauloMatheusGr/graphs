...
from ablation_prep import ROI_FILTER_DEFAULT, SLOT_ORDER
...
def baseline_clinical_table(df_long: pd.DataFrame) -> pd.DataFrame:
    """1 linha/paciente, só 1ª visita (slot baseline ou t0)."""
    sub = df_long.copy()
    if "slot" in sub.columns:
        ord_ = sub["slot"].astype(str).map(SLOT_ORDER)
        sub = sub[ord_.eq(0)]
        if sub.empty:
            raise ValueError(
                "nenhuma visita baseline/t0 em df_long; "
                f"slots={sorted(df_long['slot'].astype(str).unique())}"
            )
    ...
    for fold, (tr_rel, te_rel) in enumerate(outer_cv.split(np.zeros(len(y)), y), start=1):
        ...
        if wide[list(CLINICAL_COLS)].isna().any().any():
            raise ValueError(
                ...
                f"verifique MMSE/ADAS/FAQ/CDR na visita t0/baseline"
            )
