"""exp1: ROCKET + Ridge a partir de exp1.md (pipeline minimalista).

ROCKET não tem épocas; curva extra: acurácia (validação) vs alpha em RidgeClassifier.
"""
...
def main() -> None:
    ...
    # sktime Rocket: (n_instances, n_channels, n_timepoints)
    ...
    clf = RidgeClassifierCV(alphas=ALPHAS, cv=None)
    clf.fit(Z_tr, y[tr_fit_idx])

    fig, ax = plt.subplots()
    ax.bar(["Raw", "Após correlação", "Após variância"], [n_raw, n_after_corr, n_after_var])
    ax.set_ylabel("Nº atributos")
    fig.tight_layout()
    fig.savefig(OUT_DIR / "exp1_rocket_feature_counts.png", dpi=120)
    plt.close(fig)

    acc_alpha: list[float] = []
    for a in ALPHAS:
        m = RidgeClassifier(alpha=float(a), random_state=RANDOM_STATE)
        m.fit(Z_tr, y[tr_fit_idx])
        acc_alpha.append(accuracy_score(y[val_idx], m.predict(Z_val)))

    fig2, ax2 = plt.subplots()
    ax2.plot(np.log10(ALPHAS), acc_alpha, marker="o")
    ax2.set_xlabel("log10(alpha)")
    ax2.set_ylabel("acurácia (validação)")
    fig2.suptitle("Ridge em features ROCKET (sem épocas de treino).")
    fig2.tight_layout()
    fig2.savefig(OUT_DIR / "exp1_rocket_ridge_alpha.png", dpi=120)
    plt.close(fig2)

    acc_te = accuracy_score(y[test_idx], clf.predict(Z_te))
    print(f"Acurácia teste (RidgeClassifierCV, 1º fold SGK): {acc_te:.4f}")
